...
export class BookingService {
    ...
    submitBooking = (state: IState): boolean => {
        ...
        this.bookingClient.createBooking('https://reflectionsbooking.herokuapp.com/bookings', bookingModel);
    ...
    listAllBookingsOfTypeAndDate = async (type: BookingType, date: string): Promise<Moment[]> => {
        ...
        await this.bookingClient.allBookingsByTypeAndDate('https://reflectionsbooking.herokuapp.com/bookings', type, date)
            .then(bookings => {
                for (let booking of bookings) {
                    const bookingTime = moment(`${date} ${booking.time}`);
