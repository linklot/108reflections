...
export default class Bookings extends React.Component<IProps, IState> {
  ...
  constructor(props: IProps) {
    super(props);
    const _now = moment(new Date(), 'YYYY-MM-DD');
    this.state = {
      bookingMode: this.props.bookingMode === undefined ? BookingType.NA : this.props.bookingMode,
      hideForm: true,
      hideNote: true,
      bookingValidFirstDay: _now.clone().add(1, 'days'),
      bookingValidLastDay: _now.clone().add(2, 'months'),
      bookingDate: _now.clone().add(1, 'days'),
      bbqValidFromTime: _now.clone().set('hour', 8).set('minute', 0),
      bbqValidToTime: _now.clone().set('hour', 22).set('minute', 0),
      moveValidFromTime: _now.clone().set('hour', 8).set('minute', 0),
      moveValidToTime: _now.clone().set('hour', 18).set('minute', 0),
      validFromTime: _now,
      validToTime: _now,
      bookingTime: _now,
      bookingDuration: 30,
  ...
  changeBookingMode = (bookingMode: BookingType) => (event: any) => {
    ...
    const _fromTime =
      bookingMode === BookingType.BBQ ? this.state.bbqValidFromTime : this.state.moveValidFromTime;
    const _note =
      bookingMode == BookingType.BBQ
        ? ReactHtmlParser(this.bookingNotes.bbqNote)
        : ReactHtmlParser(this.bookingNotes.moveNote);

    const _now = moment(new Date(), 'YYYY-MM-DD');
    const _bookingValidFromDate =
      bookingMode === BookingType.BBQ ? _now.clone().add(1, 'days') : _now.clone().add(2, 'days');

    this.bookingService
      .listAllBookingsOfTypeAndDate(bookingMode, this.state.bookingDate.format('YYYY-MM-DD'))
      .then((data) => {
        this.setState({
          bookingMode: bookingMode,
          hideNote: false,
  ...
  selectBookingDate = (date: Moment) => {
    ...
    this.bookingService.listAllBookingsOfTypeAndDate(bookingType, bookingDate).then((data) => {
      const dayToday = date.isoWeekday();
      const _now = moment(new Date(), 'YYYY-MM-DD');
      const moveStartTime = (() => {
        switch (dayToday) {
          case 6:
            return _now.clone().set('hour', 9).set('minute', 0);
          case 7:
            return _now.clone().set('hour', 0).set('minute', 0);
          default:
            return this.state.moveValidFromTime;
        }
      })();
      const moveEndTime = (() => {
        switch (dayToday) {
          case 6:
            return _now.clone().set('hour', 13).set('minute', 0);
          case 7:
            return _now.clone().set('hour', 0).set('minute', 0);
          default:
            return this.state.moveValidToTime;
        }
      })();

      const possibleStartTime =
        this.state.bookingMode === BookingType.BBQ ? this.state.bbqValidFromTime : moveStartTime;
      ...
      this.setState({
        ...
        validFromTime:
          this.state.bookingMode === BookingType.BBQ ? this.state.bbqValidFromTime : moveStartTime,
        validToTime:
          this.state.bookingMode === BookingType.BBQ ? this.state.bbqValidToTime : moveEndTime,
      });
    });
  };
  ...
  calculateBookingStartTime = (possibleStartTime: Moment, bookedTimes: Moment[]): Moment => {
    ...
    if (_bookedTimes.indexOf(_possibleStartTime) === -1) {
      ...
      return this.calculateBookingStartTime(
        possibleStartTime.clone().add(30, 'minutes'),
        bookedTimes
      );
    }
  };
  ...
  public render() {
    ...
    return (
      <div>
        <header
          className='w3-display-container w3-content w3-wide w3-margin-bottom'
          style={maxWidthStyle}
        >
          <img
            className='w3-image'
        ...
        <div className='w3-content w3-margin-top' style={maxWidthStyle}>
          <div className='w3-container w3-margin-top'>
            <div className='w3-col l12 m12 s12 w3-margin-bottom'>
              ...
              <p>
                Once you submit your booking request, an automatic email will be sent to you
                acknowledging receipt of that. Another confirmation email will be sent to you after
                your booking is processed. If you do not receive any automatic email in your inbox
                or junk mail folder, the booking request may have not been submitted successfully.
                In that case, you may try again or email us on{' '}
              ...
              {/* <p>The BBQ is open now.<br/>
                                ...
                                <ul>
                                    ...
                                    <li>Clean the BBQ and the surrounding areas before and after use</li>
              ...
              <form name='bookingForm'>
                <div className='w3-col l12 m12 s12'>
                  ...
                  <div className='w3-col l6 m12 s12 w3-margin-bottom'>
                    ...
                    <div className='w3-col l12 m12 s12'>
                      ...
                      <select
                        className='w3-border'
                        onChange={this.selectBookingDuration}
                ...
                <p>
                  <label className={css.formLabel}>
                    Apartment Number <span className={css.required}>*</span>
                  </label>
                  <input
                    className={`w3-input ${this.state.invalidApartmentName ? 'w3-border-red' : ''}`}
                    type='text'
                    ref={(e) => (this._apartmentNumber = e)}
                  />
                </p>
                ...
                <p>
                  ...
                  <input
                    className={`w3-input ${this.state.invalidName ? 'w3-border-red' : ''}`}
                    type='text'
                    ref={(e) => (this._name = e)}
                  />
                </p>
                ...
                <p>
                  <label className={css.formLabel}>
                    Phone / Mobile <span className={css.required}>*</span>
                  </label>
                  <input
                    className={`w3-input ${this.state.invalidPhone ? 'w3-border-red' : ''}`}
                    type='text'
                    ref={(e) => (this._phone = e)}
                  />
                </p>
                ...
                <p>
                  ...
                  <input
                    className={`w3-input ${this.state.invalidEmail ? 'w3-border-red' : ''}`}
                    type='text'
                    ref={(e) => (this._email = e)}
                  />
                </p>
                ...
                <div>
                    ...
                    <div className='w3-modal-content w3-card-4 w3-animate-opacity'>
                        ...
                        <table className='w3-table w3-striped'>
                          <tbody>
                            <tr>
                              <td>Date&amp;Time:</td>
                              <td>
                                {this.state.bookingDate.format('DD MMM YYYY')}{' '}
                                {this.state.bookingTime.format('h:mm a')}
                              </td>
                            </tr>
                            <tr>
                              <td>Duration:</td>
                      ...
                      <footer className='w3-container'>
                        <p className='w3-right'>
                          <button
                            className='w3-button w3-green w3-hover-teal w3-round-large'
                  ...
                  {/* Notification Modal */}
                  <div
                    className={`w3-modal ${this.state.formSubmitted ? css.showConfimModal : ''}`}
                  >
                    <div className='w3-modal-content w3-card-4 w3-animate-opacity'>
                      <header className='w3-container w3-teal'>
                        <span
                          className='w3-button w3-display-topright'
                          onClick={this.hideNotifyModal}
                        >
                          &times;
                        </span>
                      ...
                      <div className='w3-container'>
                        <p>Thank you!</p>
                        <p>
                          Your booking has been successfully submitted. We will send an email to
                          confirm.
                        </p>
                      </div>
                    </div>
